...
export class BotBlazeWithTelegram {
	constructor(options) {
		...
		this.simulator = new Simulator(Boolean(appConfig("enterProtection", false))); // options.enterProtection
		...
		this.gale = {
			...
			isGale: false,
			crashPoint: 0,
			maxGales: Number(appConfig("maxGales", 3)),
		};
	...
	async run() {
		...
		if (Boolean(appConfig("allowAutoBet", false))) {
			...
			this.simulator.setInitialData({
				...
				betAmount: Number(appConfig("betAmount", 0.1)),
				autoCrashAt: Number(appConfig("autoCrashAt", 1.2)),
				betNetProfitAmount: Number(appConfig("betNetProfitAmount", 0.03))
			});
		}
		...
		this.blaze.ev.on("game_waiting", async (data) => {
			console.log(
		...
		this.blaze.ev.on("game_graphing", async (data) => {
			data &&
				console.log(
					chalk.cyan(`[${new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour12: true, hour: '2-digit', minute: '2-digit', second: '2-digit'})}]`),
					chalk.yellow("Status:"),
					"Apostas encerradas e crash subindo!"
				);
			// this._summary({ verifyDate: true });
		});
		...
		this.blaze.ev.on("game_complete", async (data) => {
			data &&
				console.log(
					...
					// "Girando... Resultado:",
					"Crash:",
					`[${chalk.yellow(data.crash_point + "x")}]`
				);
	...
	async invokeAnalyst(data) {
		// let { crash_point } = data;
		...
		let crashPoint = this?.gale?.crashPoint || 0;
		this.simulator.makeBet(crashPoint);
	...
	async invokeResult(data) {
		...
		let { crash_point } = data;
		if (typeof crash_point !== "undefined" && crash_point !== null) {
			if (crash_point >= 0) {
				this.gale.crashPoint = crash_point;
			}
			...
			if (crash_point) this.simulator.storeResult(crash_point);
